...
from cf_remote.utils import pretty, is_in_past, canonify, file_extension, file_name
...
class Artifact:
    def __init__(self, data, filename=None):
        ...
        self.file_name = file_name(self.url)
        self.file_extension = file_extension(self.file_name)

        self.tags = ["any"]

        self.add_tag(self.arch)
        self.add_tag(self.file_extension)
        ...
        parts = re.split(r"[-_.]", self.file_name)
    ...
    def __str__(self):
        return self.file_name + " ({})".format(" ".join(self.tags))
